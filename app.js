...
function renderMap() {
  ...
  const terminus = `
    ...
    <text class="map-label strong" x="${start.x - 14}" y="${start.y + 30}">Depart: total beginner</text>
    <circle cx="${end.x}" cy="${end.y}" r="9" fill="var(--porcelain)"></circle>
    <circle cx="${end.x}" cy="${end.y}" r="4" fill="${LINE_INK.p9}"></circle>
    <text class="map-label strong" x="${end.x + 16}" y="${end.y + 28}">Terminus: paid-ads operator</text>`;
...
function setupChrome() {
  ...
  $("#export-btn").addEventListener("click", () => {
    ...
    a.download = `ads-backup-${iso(today())}.json`;
    a.click();
